<script lang="ts">
    ...
    let yBounds = $derived.by(() => {
        ...
        const allVals = candles.flatMap((c) => [
            c.high,
            c.low,
            c.open,
            c.close,
    ...
    function candleX(i: number): number {
        ...
        return (
            PAD_LEFT +
            (i / (candles.length - 1)) * (chartW - candleWidth) +
            candleWidth / 2
        );
    }
    ...
    let crosshairYPx = $derived(
        mouseY != null
            ? Math.max(PAD_TOP, Math.min(PAD_TOP + chartH, mouseY))
            : null,
    );
...
<div class="relative w-full h-full flex flex-col" style="min-height: 0;">
    <div class="flex items-baseline justify-end px-2 py-1 shrink-0">
        ...
        >
            {#if tooltipCandle}
                O:{formatPaceDisplay(tooltipCandle.open, units)} C:{formatPaceDisplay(
                    tooltipCandle.close,
                    units,
                )}
        ...
        {#each candles as candle, i (i)}
            {#if i % Math.max(1, Math.floor(candles.length / 8)) === 0 || i === candles.length - 1}
                <text
                    ...
                    fill="var(--term-text-muted)"
                    font-size="10"
                    font-family="'Geist Mono', monospace">{candle.label}</text
                >
            {/if}
        {/each}
    </svg>
    ...
    {#if tooltipCandle}
        ...
        <ChartOverlay left={PAD_LEFT + 2}>
            <div style="color: var(--term-text-bright); line-height: 1.6;">
                <div>
                    <span style="color: var(--term-text-muted);">Open</span>
                    {formatPaceDisplay(c.open, units)}
                </div>
                <div>
                    <span style="color: {isGreen ? '#22c55e' : '#ef4444'};"
                        >High</span
                    >
                    {formatPaceDisplay(c.high, units)}
                </div>
                <div>
                    <span style="color: {isGreen ? '#ef4444' : '#22c55e'};"
                        >Low</span
                    >
                    {formatPaceDisplay(c.low, units)}
                </div>
                <div>
                    <span style="color: var(--term-text-muted);">Close</span>
                    {formatPaceDisplay(c.close, units)}
                </div>
            </div>
        </ChartOverlay>
    {/if}
